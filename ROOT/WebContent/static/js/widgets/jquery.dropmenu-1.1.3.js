...
(function($, undefined) {
    $.fn.dropmenu = function(options) {
        ...
        return this.each(function() {
            ...
            var opts  = $.extend({}, $.fn.dropmenu.defaults, options), $menu = $(this), $topl = $menu.find('> li'), menuX = $menu.offset().left;
            ...
            //	all A's and SPANs
            $menu.find('li > a, li > span').css(css);


            //	top-level LI's and top level A's and SPANs            
            if(isIE)
                $topl.css('float', 'left').find('.topa').addClass('toplevel').css('float', 'left');
            else
                $topl.css('float', 'left').find('> a, > span').addClass('toplevel').css('float', 'left');
            ...
            .addClass('sublevel').parent().find('ul').css({
                top	: 0
            }).data('subsub', true);
            ...
            //	IE fixes
            if (isIE6) {               
                $menu.find('ul').css('line-height', '0');
            }

            if (isIE6 || isIE7) {              
                $menu.find('ul a, ul span').css('zoom','1');               
            }


            $menu.find('a').click(function() {
                $('ul', $menu).hide();
                $('a, span', $menu).removeClass('hover').removeClass('selected');
            ...
            $menu.find('li').hover(            
                ...
                function() {                   
                    var listit = this,
                    subnav = $.getSubnav(listit),
                    subcss = {
                        zIndex: $.fn.dropmenu.zIndex++
                    };
                    if(isIE)
                        $(listit).find('.topa').addClass('hover');
                    else
                ...
                //	hiding submenu
                    ...
                    if (!subnav) {
                        if(isIE)
                            $(listit).find('.topa').removeClass('hover');
                        else
